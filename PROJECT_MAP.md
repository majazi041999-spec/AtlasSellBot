...
**A CDN edge is not a customer, and this nearly fired on real people.** Three of the five live servers front their inbound with Cloudflare, so the address xray records is a Cloudflare edge, never the customer — and one customer's connections land on dozens of edges. A single honest subscription showed **76 simultaneous "places"** and was one cycle from a sharing warning. `_CDN_CIDRS` (Cloudflare's published ranges, plus `ip_limit_cdn_extra` for ArvanCloud/Derak/etc.) is checked in `group_key`, so CDN addresses are dropped exactly like private ones. Measured before and after on the live fleet: **19 subscriptions "over the limit" became 0**, and the worst offender went from 76 places to 5. Every one was an artifact. Counting these would have inverted the feature — the customers punished hardest would be the ones on the servers that work best in Iran. `tests/test_ip_guard.py` §25 pins this using the owner's actual addresses.

**The cost is real and is reported, not hidden.** A CDN-fronted server cannot be policed at all, so on this fleet the limit covers **2 servers out of 5**. `coverage()` reports which, the settings card says so in plain language, `tools/diagnose.py` flags it per server, and any reading that includes a CDN-fronted server is marked `partial`. Fixing it properly is server-side work (nginx reading `CF-Connecting-IP` and forwarding PROXY protocol to an inbound with `acceptProxyProtocol`), not something this repo can do.

**Earlier measurement, now corrected:** an initial reading reported 20 subscriptions over the allowance with a tail at 28-58 places and attributed it to link sharing. That was wrong — it was this CDN artifact, and the corrected count is zero.

**A carrier NAT is not twenty customers, and this one HAD already fired.** After the CDN fix a subscription still showed 15 simultaneous places, all inside `31.171.96.0/21` — Delta Telecom Azerbaijan (AS29049), a leased block with no PTR records. It is a carrier NAT, proven four ways: one address served two unrelated subscriptions minutes apart (a device cannot, a gateway does it constantly); one subscription showed 30 distinct addresses from it in 150 seconds with only 7 present throughout; 31 unrelated subscriptions egress from the same /23; and the worst-affected one had used 6 GB in a month on an unlimited plan, which is not twenty people. **Of 164 enforcement events, 160 came from this pool — including all 29 cuts.** Every customer the guard ever cut was innocent, and 15 were still carrying a penalty rung earned entirely by the artifact when it was found (cleared; `#789` was at level 4, one trip from the top of the ladder).

So the guard now LEARNS shared gateways rather than excluding them. Excluding the block the way CDN ranges are excluded would be wrong in the other direction — those addresses would count as zero and real sharing from there would be free. Instead `group_key` collapses every address inside a known gateway to ONE key, so a carrier NAT is one place however many addresses come out of it. The evidence is the signature above: an address seen serving two **different customers** — distinct customers, never distinct subscriptions, because one person using two of their own subs from one home router shares an address quite legitimately and widening their neighbours' /24 for that would help nobody. A proven address widens only to its /24. Seeded with the Baku pool so the fix does not wait on a scan; `detect_gateways()` re-derived it independently within minutes of going live, from 8 addresses each serving two customers. Cost: one batched upsert per cycle (11.7 ms for 300 addresses) and one GROUP BY per ten minutes (5.1 ms), no extra panel requests. The panel lists each network with the address that proved it; a block the owner disables stays disabled when detection finds it again. `tests/test_ip_guard.py` §28-34.

**A second earlier measurement, also corrected:** `#566 Br-Gorbah` was recorded here as the one genuine sharer at 71 places. It is 52 pool addresses against 4 others — the same artifact. There are currently **no confirmed sharers on the fleet at all.**
...
## 6f. Diagnosing the quiet failures (`tools/diagnose.py`, `ip_guard.diagnose`)
Three of this project's features fail by simply not happening: an AI model id that no longer exists, a background worker that was never restarted, and a setting that is off. From the outside all three look identical, and the owner has no way to tell which. So they answer the question themselves rather than requiring someone to log into the server.
- `python tools/diagnose.py [profile_id]` — run on the server. Reports the AI settings and asks the key which models it can actually use, the guard's settings and whether its worker has run recently, every panel's reachability and whether `clientIpsByGuid` answers, and optionally one subscription's full verdict. **It never prints a secret** — keys, panel passwords and sub tokens are reported as present/absent and by length only, so the output is safe to paste into a chat.
...
- **Thinking eats the answer.** From Gemini 2.5 on the model reasons before replying and those tokens come out of `maxOutputTokens`. At the original 2048 it spent the budget thinking and returned truncated JSON, which surfaced as `bad_output` — a message that blamed the model's manners rather than our request. `thinkingConfig.thinkingBudget=0` is sent (with a retry without it for models that reject the field), the budget is 8192, parts marked `thought` are skipped, and `finishReason` now drives distinct `truncated` / `empty_output` / `blocked` errors instead of one useless catch-all.
- **A 404 from Gemini is never the key and never the URL** — it means the project has no model by that name, and model ids are Google's to rename and retire on their own schedule, so any name hard-coded here is a future 404. `list_models()` asks the key what it actually has, the 404 handler names those models in the error, and the settings page offers them as buttons. `_clean_model()` strips the `models/` prefix, because that is how Google's console displays an id and therefore how people paste it. Covered by `tests/test_ai_analyst.py`.
- Endpoints: `GET /{S}/api/analytics/ai` (runs it), `GET /{S}/api/analytics/ai/models` (what this key can use) and `GET /{S}/api/analytics/ai/status` (is it configured — used to decide whether to show the panel at all). The API key is write-only: it is never returned to the browser, and an empty field on save leaves the stored key untouched.
