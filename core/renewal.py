...
async def find_and_renew_config(cfg: Dict, traffic_gb: float, duration_days: int) -> Dict:
    ...
    for server in servers:
        ...
        try:
            ...
            ok = await client.update_client(inbound_id, client_uuid, cfg["email"], traffic_gb, new_expire_ms, True)
            if not ok:
                last_error = f"update failed on server {server.get('name')}"
                continue

            await client.reset_client_traffic(inbound_id, cfg["email"])
            link = await client.get_client_link(inbound_id, cfg["email"])
            sub = await client.get_subscription_link(inbound_id, cfg["email"])
    ...
    return {"ok": False, "error": last_error or "config not found on any active server"}
